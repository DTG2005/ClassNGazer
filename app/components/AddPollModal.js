...
export default function AddPollModal({ onClose, onAdd, onEdit, initialData }) {
  ...
  useEffect(() => {
    ...
    const hasContent = question.trim() || topic.trim() || options.some(o => o.text.trim()) || solution.trim();
    if (hasContent) {
      localStorage.setItem('poll_draft', JSON.stringify(draft));
      setHasDraft(true);
    }
  }, [question, topic, options, correctIndices, timer, solution, isEditing]);

  // Clear draft — resets form + removes from localStorage
  const clearDraft = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Clear draft on any successful submit (live, schedule, or save draft)
      localStorage.removeItem('poll_draft');
      setHasDraft(false);
    } catch (e) {
      console.error(e);
