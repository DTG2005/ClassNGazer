              ...
              <div style={S.bannerLeft}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:'11px', color:'rgba(255,255,255,0.6)', marginTop:'2px' }}>
                    ...
                    {aq.negativeMarking && ` · −${aq.negativeValue} per wrong`}
                  </div>
                </div>
              </div>
              <div style={S.bannerRight}>
                ...
                {isProfessor ? (
                  ...
                  <button style={S.bannerBtn} onClick={() => setAttemptQuiz(aq)}>
                    {studentResponses[aq.id] ? '📋 View My Result' : '🚀 Attempt Quiz →'}
                  </button>
                )}
              </div>
            </div>
            ...
            <div style={S.grid}>
              {draftQuizzes.map(q => (
                <div key={q.id} style={{ ...S.card, borderColor:'var(--orange-dim)', borderStyle:'dashed' }}>
                  <div style={S.cardTop}>
                    <span style={S.tagDraft}>DRAFT</span>
                  ...
                  <div style={S.cardMeta}>
                    {q.totalMarks} marks · {q.negativeMarking ? `−${q.negativeValue} penalty` : 'No penalty'}
                    {q.shuffleQuestions && ' · Shuffle ✓'}
                  </div>
                  <div style={{ display:'flex', gap:'8px' }} onClick={e => e.stopPropagation()}>
                    <button onClick={() => handleStart(q.id)} style={S.startBtn}>▶ Launch</button>
            ...
            {closedQuizzes.map((q, idx) => {
              ...
              return (
                <div key={q.id} style={{ ...S.card, cursor: isProfessor || myRes ? 'pointer' : 'default' }}
                  ...
                  <div style={S.cardMeta}>
                    {q.totalMarks} marks
                    {q.negativeMarking && ` · −${q.negativeValue}/wrong`}
                  ...
                  {isProfessor && (
                    <div style={{ display:'flex', alignItems:'center', gap:'10px', paddingTop:'10px', borderTop:'1px solid var(--gray-100)' }}>
                      ...
                      <div style={{ display:'flex', gap:'4px' }} onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => setEditQuiz(q)} title="Edit"
