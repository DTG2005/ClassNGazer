...
// ── Quiz Attempt Modal ──
export default function QuizAttemptModal({ quiz, onClose, onSubmitted, studentId, studentName, existingResponse }) {
  ...
  useEffect(() => {
    if (result || submitted) return; // already submitted — no timer needed
    ...
    const unsub = quizDatabase.listenToLiveQuiz(quiz.id, (data) => {
      if (!data || data.liveStatus !== 'active') {
        ...
        if (!submittingRef.current && !submitted) handleAutoSubmit();
        return;
      }
  ...
  const toggleOption = useCallback((origQIdx, origOptIdx) => {
    ...
    const isMulti = quiz.questions[origQIdx].correctOptions.length > 1;
    setAnswers(prev => {
  ...
  const handleSubmit = async (auto = false) => {
    ...
    try {
      const res = await quizDatabase.submitQuizResponse(quiz.id, studentId, studentName, answers, quiz);
      setResult(res);
      setSubmitted(true);
                ...
                <div key={origQIdx} style={{ marginBottom:'14px', border:'1px solid var(--gray-100)', borderRadius:'var(--radius-md)', overflow:'hidden' }}>
                  ...
                  <div style={{ padding:'10px 14px', display:'flex', flexDirection:'column', gap:'5px' }}>
                    {opts.map((o, displayOptIdx) => {
                      ...
                      const isCorrect  = (q.correctOptions || []).includes(origOptIdx);
                      const wasChosen  = (pq?.answered || []).includes(origOptIdx);
                  ...
                  {(q.solution || q.solutionImage) && (
                    <div style={{ margin:'0 14px 12px', padding:'10px 12px', background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:'8px' }}>
                      ...
                      {q.solutionImage && <img src={q.solutionImage} style={{ maxHeight:'80px', borderRadius:'5px', marginTop:'6px' }} />}
                    </div>
                  )}
                </div>
              );
              ...
              <div key={origQIdx}>
                ...
                <div style={{ display:'flex', gap:'12px', padding:'14px 16px', background:'var(--gray-50)', alignItems:'flex-start', border:'1px solid var(--gray-100)', borderBottom:'none', borderTopLeftRadius:'var(--radius-md)', borderTopRightRadius:'var(--radius-md)' }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    {q.questionImage && <img src={q.questionImage} style={{ maxHeight:'160px', borderRadius:'8px', marginTop:'12px' }} />}
                    ...
                    return (
                      <div key={origOptIdx}
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {o.image && <img src={o.image} style={{ maxHeight:'100px', borderRadius:'6px', display:'block', marginTop:'6px' }} />}
                        </div>
                      </div>
                    );
                  })}
