...
export default function AddQuizModal({ onClose, onAdd, onEdit, initialData }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (d.timerMins != null) setTimerMins(String(d.timerMins));
        if (d.timerSecs != null) setTimerSecs(String(d.timerSecs));
        if (d.negativeMarking != null) setNegativeMarking(d.negativeMarking);
        if (d.negativeValue != null)   setNegativeValue(d.negativeValue);
        if (d.shuffleQ != null)  setShuffleQ(d.shuffleQ);
        if (d.shuffleOpts != null) setShuffleOpts(d.shuffleOpts);
        if (d.questions?.length) {
          setQuestions(d.questions.map(q => ({
            text: q.text || '',
            questionImage: null, // images can't be serialised to localStorage
            options: (q.options || []).map(o => ({ text: o.text || '', image: null })),
            correctOptions: q.correctOptions || [],
            marks: q.marks != null ? String(q.marks) : '1',
            negativeMarks: q.negativeMarks != null ? String(q.negativeMarks) : '0',
            solution: q.solution || '',
            solutionImage: null,
          })));
        }
  ...
  useEffect(() => {
    if (isEditing) return;
    const hasContent = title.trim() || questions.some(q => q.text.trim() || q.options.some(o => o.text.trim()));
    if (!hasContent) return;
    const draft = {
      title, timerMins, timerSecs,
      negativeMarking, negativeValue,
      shuffleQ, shuffleOpts,
    ...
    setHasDraft(true);
  }, [title, timerMins, timerSecs, negativeMarking, negativeValue, shuffleQ, shuffleOpts, questions, isEditing]);

  // ── Clear draft — resets all fields + removes from localStorage ──
  const clearDraft = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        await onAdd(payload);
        // Clear draft on any successful submit
        localStorage.removeItem('quiz_draft');
        setHasDraft(false);
      }
    } catch (e) {
    ...
    <div style={ov} onClick={onClose}>
      <div style={card} onClick={e => e.stopPropagation()}>
        ...
        <div style={bdy}>
          ...
          {/* Clear Draft banner — shown at top when draft is restored */}
          {!isEditing && hasDraft && (
            <div style={{ padding: '10px 14px', background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '14px' }}>📝</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: '#92400E' }}>Draft restored from last session</span>
              </div>
              <button
                type="button"
                onClick={clearDraft}
                style={{ fontSize: '11px', fontWeight: 700, color: '#B45309', background: '#FEF3C7', border: '1px solid #FDE68A', borderRadius: '6px', padding: '4px 10px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                Clear Draft
              </button>
            </div>
          )}
