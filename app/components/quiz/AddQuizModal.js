...
function QuestionEditor({ q, idx, onChange, onRemove, canRemove, preview }) {
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          <label style={{ fontSize:'11px', fontWeight:700, color:'var(--gray-600)', display:'flex', alignItems:'center', gap:'4px' }}>
            ...
            <input type="number" min="0.5" step="0.5" value={q.marks}
              ...
              style={{ width:'54px', padding:'4px 6px', border:'1px solid var(--gray-200)', borderRadius:'6px', fontSize:'12px', fontFamily:'DM Mono', textAlign:'center', background:'var(--white)' }} />
          </label>
          {canRemove && (
...
export default function AddQuizModal({ onClose, onAdd, onEdit, initialData }) {
  ...
  const [timerMins,       setTimerMins]       = useState(Math.floor((initialData?.timer || 600) / 60));
  const [timerSecs,       setTimerSecs]       = useState((initialData?.timer || 600) % 60);
  ...
  // Sync slider → mm:ss
  useEffect(() => {
    setTimer(timerMins * 60 + timerSecs);
  }, [timerMins, timerSecs]);
  ...
  const handleSubmit = async () => {
    ...
    if (timer < 30) { alert('Minimum quiz duration is 30 seconds.'); return; }
    ...
    try {
      ...
      const payload = {
        ...
        timer,
        negativeMarking,
        negativeValue:    negativeMarking ? Number(negativeValue) : 0,
  ...
  const fmtTimer   = `${String(timerMins).padStart(2,'0')}:${String(timerSecs).padStart(2,'0')}`;
            ...
            <div>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginTop:'6px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'4px', background:'rgba(255,255,255,0.08)', borderRadius:'8px', padding:'6px 12px', border:'1px solid rgba(255,255,255,0.15)' }}>
                  <input type="number" min="0" max="180" value={timerMins} onChange={e => setTimerMins(Number(e.target.value))}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'4px', background:'rgba(255,255,255,0.08)', borderRadius:'8px', padding:'6px 12px', border:'1px solid rgba(255,255,255,0.15)' }}>
                  <input type="number" min="0" max="59" value={timerSecs} onChange={e => setTimerSecs(Number(e.target.value))}
                    style={{ width:'40px', background:'transparent', border:'none', outline:'none', color:'white', fontSize:'22px', fontWeight:800, fontFamily:'DM Mono', textAlign:'center' }} />
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'14px', flexWrap:'wrap' }}>
              ...
              {negativeMarking && (
                <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                  <span style={{ fontSize:'11px', color:'rgba(255,255,255,0.6)' }}>Deduct</span>
                  <input type="number" min="0" step="0.25" value={negativeValue}
                    onChange={e => setNegativeValue(e.target.value)}
                    style={{ width:'60px', padding:'4px 8px', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:'6px', color:'white', fontSize:'13px', fontFamily:'DM Mono', textAlign:'center', outline:'none' }} />
                  <span style={{ fontSize:'11px', color:'rgba(255,255,255,0.6)' }}>marks per wrong</span>
                </div>
              )}
            </div>
