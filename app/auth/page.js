...
export default function AuthPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      const gUser = await authService.googleSignIn();
      if (gUser.isNewUser) { setGoogleUser(gUser); setName(gUser.name || ''); setEmail(gUser.email || ''); setMode('pickrole'); }
      // If not a new user, AuthContext automatically triggers and useEffect handles the router.push
    } catch (e) {
  ...
  const handleEmailSignUp = async (e) => {
    ...
    try { 
      ...
      await refreshAuth(); // Manually refresh because onAuthStateChanged might have missed the new profile
    }
  ...
  const handlePickRole = async () => {
    ...
    try {
      ...
      await refreshAuth(); // Trigger context update for new user
