...
export const quizDatabase = {
  ...
  // ── Add time to active quiz ──
  async addTime(quizId, extraSeconds) {
    try {
      // Update Firestore
      await updateDoc(doc(db, 'quizzes', quizId), {
      ...
      const liveQuizRef = ref(realtimeDb, `liveQuizzes/${quizId}`);
      const snap = await get(liveQuizRef);
      if (snap.exists()) {
        const curr = snap.val();
        await set(ref(realtimeDb, `liveQuizzes/${quizId}/timer`), (curr.timer || 0) + extraSeconds);
      }
      return true;
