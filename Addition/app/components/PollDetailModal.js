...
export default function PollDetailModal({ poll, onClose, role, onClosePoll, user, onEdit }) {
  ...
  // ── CONTROLLED RESULT VISIBILITY (SRS 3.2.8) ──
  // Professor: ALWAYS sees results
  // Student: ONLY after poll closes — NOT while active, NOT after submitting
  const showResults = (role === 'professor' || pollClosed) && poll.status !== 'draft';
  const showCorrect = role === 'professor' || pollClosed; // highlight correct answer always for prof
  ...
  return (
    <div style={S.ov} onClick={onClose}><div style={S.card} onClick={e => e.stopPropagation()}>
      ...
      <div style={{ padding: '16px 24px 20px', borderBottom: '1px solid var(--gray-100)' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 700, lineHeight: 1.4, color: 'var(--ink)' }}>{livePoll.question}</h2>
          ...
          return (
            <div key={o.id} onClick={() => canClick && setSelectedOption(o.id)}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: 500, color: 'var(--ink)' }}>{o.text}</span>
                {/* Correct tag — only when results visible */}
              ...
              {/* Percentage bars — only when results visible */}
              {showResults && (
      ...
      {/* ── Solution (shown only when poll is closed) ── */}
      {(pollClosed || poll.status === 'draft') && poll.solution && (
        <div style={{ margin: '0 24px 20px', padding: '16px 18px', borderRadius: 'var(--radius-md)', background: '#FFFBEB', border: '1px solid #FDE68A' }}>
          ...
          <div style={{ fontSize: '14px', color: '#78350F', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{poll.solution}</div>
        </div>
      )}
      ...
      {role === 'professor' && (
        ...
        <div>
          {poll.status !== 'draft' && <>
            <div style={{ fontSize: '11px', color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase' }}>Accuracy</div>
            <div style={{ fontSize: '32px', fontWeight: 800, color: 'var(--orange)' }}>
              {correctOption && liveCount > 0 ? `${Math.round((correctOption.votes / liveCount) * 100)}%` : '—'}
            </div>
          </>}
        </div>
