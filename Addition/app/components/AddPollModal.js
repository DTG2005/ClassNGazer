  ...
  return (
    <div style={ov} onClick={onClose}><div style={card} onClick={e => e.stopPropagation()}>
      ...
      <div style={bdy}>
        ...
        <div style={fld}><label style={lbl}>Question</label>
          <textarea style={{ ...inp, resize: 'none', lineHeight: 1.5 }} placeholder="Enter your poll question..." value={question} onChange={e => setQuestion(e.target.value)} rows={3} /></div>

        <div style={fld}><label style={lbl}>Answer Options <span style={opt}>— click ✓ to mark correct</span></label>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>{options.map((o, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', border: `2px solid ${correctIdx === i ? '#86EFAC' : 'var(--gray-100)'}`, borderRadius: 'var(--radius-md)', background: correctIdx === i ? '#F0FDF4' : 'var(--gray-50)' }}>
              ...
              <input style={{ flex: 1, fontSize: '13px', color: 'var(--ink)', background: 'transparent', border: 'none', outline: 'none', fontFamily: 'Sora' }} placeholder={`Option ${LABELS[i]}`} value={o} onChange={e => { const n = [...options]; n[i] = e.target.value; setOptions(n); }} />
        ...
        <div style={fld}><label style={lbl}>Solution <span style={opt}>optional — shown after poll ends</span></label>
          <textarea style={{ ...inp, resize: 'none', lineHeight: 1.5 }} placeholder="Explain the correct answer, add context, formula, or reference..." value={solution} onChange={e => setSolution(e.target.value)} rows={3} /></div>
