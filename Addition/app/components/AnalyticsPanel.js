      ...
      <div style={{ padding: '20px 24px 24px' }}>
        {loading ? <p style={{ color: 'var(--gray-400)', fontSize: '13px', textAlign: 'center', padding: '20px' }}>Loading analytics...</p> : <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '300px', overflowY: 'auto' }}>
            {polls.map((p, i) => {
              const correct = p.options?.find(o => (p.correctOptions || [p.correctOption]).map(Number).includes(p.options.indexOf(o)));
              ...
              return (
                <div key={p.id} style={{ padding: '10px 12px', background: 'var(--gray-50)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--gray-100)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                    <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--ink)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginRight: '8px' }}>Q{i + 1}: {p.question}</span>
